...
class Transaksi_model extends CI_Model
{
    ...
    public function getRarelyOrderedItems()
    {
        $query = $this->db->query("
            SELECT m.nama_menu as nama_makanan, COALESCE(SUM(mb.jumlah), 0) as total_terjual 
            FROM menu m
            LEFT JOIN menu_dibooking mb ON m.nama_menu = mb.nama_makanan
            GROUP BY m.nama_menu
            HAVING COALESCE(SUM(mb.jumlah), 0) < 5
            ORDER BY total_terjual ASC
        ");
        return $query->result();
    }

    // Fungsi untuk mendapatkan menu populer juga perlu dimodifikasi
